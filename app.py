    ...
    try:
        ...
        for comp in components:
            cursor.execute("""
                ...
                ELSE
                BEGIN
                    INSERT INTO Components (panel_serial, section_name, component_name, make, serial_number, time)
                    VALUES (?, ?, ?, ?, ?, GETDATE())
                END
...
def export_excel():
    ...
    try:
        ...
        comp_df = pd.read_sql("SELECT section_name, component_name, make, serial_number, time FROM Components WHERE panel_serial = ?", conn, params=[panel_serial])
        conn.close()
        ...
        # Define component order based on Product Type
        order = []
        if panel_data['product_type'] == "DPS":
            # Simplified order logic for DPS
            order = ["Enclosure", "Fan Box", "Magnetics", "Switchgears", "Sensors", "Resistors", "PCB", "Filter", "Capacitor", "Stack-1", "Stack-2", "Stack-3", "Stack-4"]
        else:
            order = ["Enclosure", "Fan Box", "Magnetics", "Switchgears", "Sensors", "Resistors", "PCB", "Capacitor", "Power Supply", "U1 STACK", "V1 STACK", "W1 STACK", "U2 STACK", "V2 STACK", "W2 STACK"]

        # Sort the components by section order
        ...
        with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
            ...
            sheet.write(3, 0, "End Date")
            sheet.write(3, 1, str(panel_data.get('end_date', '')))
            ...
            sheet.write(8, 0, "Approved By")
            sheet.write(8, 1, panel_data.get('approved_by', ''))
            ...
            for row_num, row_data in enumerate(comp_df.values):
                ...
                sheet.write(12 + row_num, 4, str(row_data[4]).split('.')[0]) # Time
